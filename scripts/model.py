...
def solveT(par):  # solve static last period consumption choice
    ...
    for im, m in enumerate(𝒢_m_det):
        ...
        res = optimize.minimize_scalar(
            ...
            bounds=[0, m + 1.0e-4],
            method="bounded",
            options={"xatol": 1e-4, "maxiter": 10000},
        )
        solvec[im] = res.x
    return solvec
...
def solve(par):  # Solves the model
    ...
    H_𝒢_MEn = (
        []
    )  # This list will consist of Cash-on-hand grids for different levels of h
    Cstar_det = np.zeros(
...
def sim(
    ...
    mc = qe.MarkovChain(
        P_markov.T, state_values=np.arange(len(H))
    )  # create a markov chain method based on P_markov to be used later
    ...
    simZ = np.zeros(
        [T + 1, N]
    )  # storage for the simulation of the productivity parameter z
    simY = np.zeros([T + 1, N])  ## Put this within the loop!!!

    simMean = np.zeros(
        [b_iter, 1]
    )  # storage for mean simM at retirement / we store it to evaluate convergence
    simStd = np.zeros(
        [b_iter, 1]
    )  # storage for mean simM at retirement / we store it to evaluate convergence
    ...
    for j in range(b_iter):
        ...
        simY[0, :] = (
            l[0] * h_children * np.exp(simZ[0, :])
        )  # E[y] = exp(μ + σ_z**2/2) // Initial Productivity from Ergodic Dist.
        ...
        for t in range(T):
            ...
            for i, h in enumerate(h_children):
                ...
                if (
                    t <= T - ch
                ):  # Our parents could still be alive // t=0,1,2, age=1,2,3
                    mp = MPguess[t + ch - 1, i]  # cash-on-hands of parents

                    # Binary Search for Parents Cash-on-hand
                    ip_min = tools.binary_search(
                        0, gridsize_m, 𝒢_M_det[t + ch - 1, :], mp
                    )
                    ip_max = ip_min + 1
                    ...
                    if (
                        ip_min == gridsize_deterministic_m - 1
                        and iz_min == gridsize_z - 1
                    ):
                        simC[t, i] = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_min, :, 0, ip_min],
                            Cstar_sto[t, iz_min, :, φi, ip_min],
                            simM[t, i],
                        )
                        ...
                        weight_Z = (simZ[t, i] - 𝒢_z[iz_min]) / (
                            𝒢_z[iz_max] - 𝒢_z[iz_min]
                        )
                        simC_min = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_min, :, φi, ip_min],
                            Cstar_sto[t, iz_min, :, φi, ip_min],
                            simM[t, i],
                        )
                        simC_max = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_max, :, φi, ip_min],
                            Cstar_sto[t, iz_max, :, φi, ip_min],
                            simM[t, i],
                        )
                        ...
                        weight = (mp - 𝒢_M_det[t + ch - 1, ip_min]) / (
                            𝒢_M_det[t + ch - 1, ip_max] - 𝒢_M_det[t + ch - 1, ip_min]
                        )
                        simC_min = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_min, :, φi, ip_min],
                            Cstar_sto[t, iz_min, :, φi, ip_min],
                        ...
                        simC_max = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_min, :, φi, ip_max],
                            Cstar_sto[t, iz_min, :, φi, ip_max],
                            simM[t, i],
                        )
                        ...
                        simC_min_Z1 = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_min, :, φi, ip_min],
                            Cstar_sto[t, iz_min, :, φi, ip_min],
                            simM[t, i],
                        )
                        simC_max_Z1 = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_max, :, φi, ip_min],
                            Cstar_sto[t, iz_max, :, φi, ip_min],
                            simM[t, i],
                        )
                        simC_Z1 = (1 - weight_Z) * simC_min_Z1 + weight_Z * simC_max_Z1
                        simC_min_Z2 = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_min, :, φi, ip_max],
                            Cstar_sto[t, iz_min, :, φi, ip_max],
                            simM[t, i],
                        )
                        simC_max_Z2 = tools.interp_linear_1d_scalar(
                            𝒢_MEn_sto[t, iz_max, :, φi, ip_max],
                            Cstar_sto[t, iz_max, :, φi, ip_max],
                            simM[t, i],
                        )
                    ...
                    Cparents = tools.interp_linear_1d_scalar(
                        𝒢_M_det[t + ch - 1, :], Cstar_det[t + ch - 1, :], mp
                    )
                    simB[t, i] = JustDead[i, t + ch] * R * (mp - Cparents)
                else:  # Parents are dead for sure // t=3,4,5, age=4,5,6
                    simC[t, i] = tools.interp_linear_1d_scalar(
                        𝒢_M_det[t, :], Cstar_det[t, :], simM[t, i]
                    )
                    simB[t, i] = np.zeros(1)
            simM[t + 1, :] = (
                w * simY[t + 1, :] + R * (simM[t, :] - simC[t, :]) + simB[t, :]
            )
            simA[t + 1, :] = simM[t, :] - simC[t, :]

        MPguess = simM  # update guess for cash-on-hand
        JustDead, DOA = simMortality(
            N, ψ
        )  # Update mortality for children, becoming parents
    ...
    return (
        ...
        hi_children,
        DOA,
    )  # gini,P90P100 # gini,BperGDP,TsimM,TsimC,hi_children,hi_parents
...
def objective(θ, targets, par, N, b_iter):
    ...
    simY, simZ, simM, simC, simA, simMean, simStd, hi_children, DOA = sim(
        𝒢_M_det, Cstar_det, H_𝒢_MEn, H_Cstar, par, N, b_iter
    )
...
def simMortality(N, ψ):
    ...
    mortal_matrix = np.random.binomial(
        size=[N, ψ.size], n=1, p=(1 - ψ) * np.ones([N, ψ.size])
    )
