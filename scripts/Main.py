...
with open(save_to + "P60P80.tex", "w") as txt_P60P80:
    txt_P60P80.write(
        "Wealth share of 60th-80th Percentile & 0.163 &"
        + format(P60P80, ".3f")
        + "\\\\"
    )
...
with open(save_to + "baseline.tex", "w") as txt_baseline:
    txt_baseline.write(
        ...
        + format(gini, ".2f")
        + "&"
        + str(WealthDistTable[0])
        + "&"
        + str(WealthDistTable[1])
        + "&"
        + str(WealthDistTable[2])
        + "&"
        + str(WealthDistTable[3])
        + "&"
        + str(WealthDistTable[4])
        + "&"
        + str(WealthDistTable[5])
        + "&"
        + str(WealthDistTable[6])
        + "\\\\"
    )
...
# )
# # no bequest motive
# par["κ"] = 0  # 4.74 # strength of bequest motivex
# # Solve the model
# Cstar_det, 𝒢_M_det, H_Cstar, H_𝒢_MEn = model.solve(par)
# # Simulate the model
# simY, simZ, simM, simC, simA, simMean, simStd, hi_children, DOA = model.sim(
